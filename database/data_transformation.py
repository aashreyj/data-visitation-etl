...
commodities_map = {
    'agriculture': ['Cashew', 'Coffee', 'Groundnut', 'Pulses', 'Sugar', 'Spices', 'Tea', 'Wheat']
}
            ...
            for commodity in commodities:
                ...
                if not data.empty:
                    ...
                    for _, row in data.iterrows():
                        ...
                        cursor_target.execute(insert_query, (row['Unit'], row['Country'], quantity, value, row['Year']))
                    
        print(f"Completed processing {data_type} for year range: {year_range} across all commodities in {supercommodity}")
        conn_target.commit()
